...
class TikTok:
    def __init__(self):
        ...
        try:
            self.amount = int(input('> Enter the Amount of views desired: '))
        except ValueError:
    ...
    def close(self, message):
        print(f'\n{message}')
        os.system("title Quinton's TikTok ViewBot - Restart required")
        os.system('pause >NUL')
        os.system("title Quinton's TikTok ViewBot - Closing...")
        sleep(3)
        os._exit(0)
    ...
    def update_title(self):
        ...
        while self.added < self.amount:
            ...
            os.system(
                f"title Quinton's TikTok ViewBot - Added: {self.added} Views/{self.amount} Views "
                f'({round(((self.added / self.amount) * 100), 3)}%) ^| Active Threads: '
                f'{threading.active_count()} ^| Time Remaining: {time_remaining}'
            )
            sleep(0.2)
        os.system(
        f"title Quinton's TikTok ViewBot - Added: {self.added}/{self.amount} "
            f'({round(((self.added / self.amount) * 100), 3)}%) ^| Active Threads: '
    ...
    def bot(self):
        ...
        if self.added > 1:
            print(f"{self.added} Views have been added!")
...
if __name__ == '__main__':
    os.system("cls && title Quinton's TikTok ViewBot")
    main = TikTok()
    main.start()
